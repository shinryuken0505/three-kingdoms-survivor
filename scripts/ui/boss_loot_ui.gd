...
static func draw(host: Node, loot: Dictionary) -> void:
	...
	var safe_margin: Vector2 = Vector2(72.0, 42.0)
	var safe_rect: Rect2 = Rect2(safe_margin, host.VIEW - safe_margin * 2.0)
	var panel_size: Vector2 = Vector2(min(980.0, safe_rect.size.x), min(594.0, safe_rect.size.y))
	var panel: Rect2 = Rect2(safe_rect.position + (safe_rect.size - panel_size) * 0.5, panel_size)
	host.draw_panel(panel, Color(0.028, 0.034, 0.032, 0.99), Color8(213, 169, 76), 2.6)

	var inner: Rect2 = panel.grow(-36.0)
	var header: Rect2 = Rect2(inner.position, Vector2(inner.size.x, 92.0))
	...
	var card_y: float = header.end.y + 20.0
	var card_h: float = 278.0
	var card_w: float = (inner.size.x - card_gap) * 0.5
	var left: Rect2 = Rect2(Vector2(inner.position.x, card_y), Vector2(card_w, card_h))
	var right: Rect2 = Rect2(Vector2(left.end.x + card_gap, card_y), Vector2(card_w, card_h))

	_draw_relic_card(host, left, loot)
	_draw_equipment_card(host, right, loot)

	var coin_rect: Rect2 = Rect2(Vector2(inner.position.x, left.end.y + 14.0), Vector2(inner.size.x, 42.0))
	host.draw_centered_text("銅錢　+%d" % int(loot.get("coins", 0)), coin_rect, 29.0, 21, Color8(239, 202, 105), true)

	var button_size: Vector2 = Vector2(340.0, 58.0)
	var button: Rect2 = Rect2(Vector2(panel.get_center().x - button_size.x * 0.5, panel.end.y - 82.0), button_size)
...
static func _draw_relic_card(host: Node, rect: Rect2, loot: Dictionary) -> void:
	...
	var relic_id: String = str(loot.get("relic", ""))
	...
	var owned: bool = bool(host.has_relic(relic_id))
	var current_level: int = int(host.relic_level(relic_id))
	var next_level: int = min(int(host.MAX_RELIC_LEVEL), current_level + 1) if owned else 1
	var rarity: String = str(rdef.get("rarity", "common"))
	var rarity_text: String = str(host.relic_rarity_name(rarity)) if host.has_method("relic_rarity_name") else rarity.to_upper()
...
static func _draw_equipment_card(host: Node, rect: Rect2, loot: Dictionary) -> void:
	...
	var equipment_id: String = str(loot.get("equipment", ""))
	...
	var equipment_name: String = "%s｜%s" % [host.equipment_slot_name(str(edef.get("slot", ""))), str(edef.get("name", equipment_id))]
