...
## 章間整備畫面的純排版模型。
##
## 所有矩形由 1280×720 基準與安全距離推導。Renderer 只讀取這些矩形，
## 不再各自手算標題、內容與按鈕位置，避免內容增加後互相壓疊。
...
const PANEL_PADDING: float = 24.0
...
static func build(view_size: Vector2 = VIEW_SIZE) -> Dictionary:
	...
	var column_width: float = (safe.size.x - COLUMN_GAP) * 0.5
	...
	var left_inner: Rect2 = inset(left, PANEL_PADDING)
	var result_height: float = 94.0
	var carry_height: float = 150.0
	var history_height: float = maxf(
		96.0,
		left_inner.size.y - result_height - carry_height - SECTION_GAP * 2.0
	)
	...
	return {
		...
		"history": history,
		"footer": footer,
		"button_row": inset(footer, Vector2(18.0, 22.0)),
	}


static func inset(rect: Rect2, amount: Variant) -> Rect2:
	var margin: Vector2 = amount if amount is Vector2 else Vector2(float(amount), float(amount))
...
static func button_rects(row: Rect2, count: int, gap: float = 12.0) -> Array[Rect2]:
	...
	if count <= 0:
		return result
	var width: float = (row.size.x - gap * float(count - 1)) / float(count)
	for index in range(count):
		result.append(Rect2(
			Vector2(row.position.x + float(index) * (width + gap), row.position.y),
