...
static func diversify_level_choices(host: Variant) -> void:
	...
	for choice_value in host.level_choices:
		if not choice_value is Dictionary:
			push_warning("Skipping invalid level choice in diversify_level_choices: type=%s value=%s" % [typeof(choice_value), str(choice_value)])
			continue
...
static func draw_integrated_hud(host: Variant) -> void:
	...
	if host.boss_spawned and not host.boss.is_empty():
		...
		var boss_resist_text: String = ElementalSynergyService.boss_resistance_text(str(host.boss.get("id", "")))
