...
const InputControllerScript = preload("res://scripts/systems/hero/hero_roster_input_controller.gd")
const RosterManagerScript = preload("res://scripts/systems/hero/hero_roster_manager.gd")
...
static func apply(
	...
	var event_id: StringName = StringName(event.get("event", InputControllerScript.EVENT_NONE))
	if event_id == InputControllerScript.EVENT_CURSOR_MOVED:
		return {"result": RESULT_CURSOR, "changed": false, "index": int(event.get("index", 0))}
	if event_id in [
		InputControllerScript.EVENT_OPEN_PICKER,
		InputControllerScript.EVENT_CLOSE_PICKER,
		InputControllerScript.EVENT_OPEN_REPLACEMENT,
		InputControllerScript.EVENT_CLOSE_REPLACEMENT,
	]:
		return {"result": RESULT_MODAL, "changed": false, "event": event_id}
	if event_id == InputControllerScript.EVENT_CLOSE_SCREEN:
		return {"result": RESULT_CLOSE, "changed": false}
	if event_id == InputControllerScript.EVENT_ALREADY_ASSIGNED:
		return {
	...
	if event_id == InputControllerScript.EVENT_MOVE_HERO:
		return _move_hero(event, active, reserve, camp, active_capacity, reserve_capacity)
	if event_id == InputControllerScript.EVENT_CONFIRM_REPLACEMENT:
...
static func _move_hero(
	...
	var moved: Dictionary = RosterManagerScript.move(
		hero_id,
		target,
		active,
		reserve,
	...
	return {
		...
		"reason": StringName(moved.get("reason", RosterManagerScript.MOVED)),
	}
...
static func _replace_hero(
	...
	var mode: StringName = StringName(event.get("mode", RosterManagerScript.ACTIVE))
	var index: int = int(event.get("index", -1))
	var pool: Array = active if mode == RosterManagerScript.ACTIVE else reserve
	...
	if mode == RosterManagerScript.ACTIVE:
		replaced = RosterManagerScript.replace_active(
			hero_id, index, active, reserve, camp, reserve_capacity
		)
	else:
		replaced = RosterManagerScript.replace_reserve(hero_id, index, active, reserve, camp)
	...
	RosterManagerScript.normalize(active, reserve, camp)
	return {
		"result": RESULT_ROSTER_CHANGED,
		...
		"replaced_to": StringName(replaced.get("replaced_target", RosterManagerScript.CAMP)),
		"reason": StringName(replaced.get("reason", RosterManagerScript.REPLACED)),
	}
