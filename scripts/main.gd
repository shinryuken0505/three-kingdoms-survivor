...
func handle_hero_config_key(key: int) -> void:
	...
	if is_up_key(key) or is_left_key(key):
		...
		var support_id: String = order[hero_config_index]
		cycle_support_assignment(support_id)
	elif is_confirm_key(key):
		var hid: String = order[hero_config_index]
		if active_heroes.has(hid):
			var destination: String = place_hero_in_support(hid)
			hero_cooldowns.erase(hid)
			show_message("%s轉入%s。" % [heroes[hid]["name"], "後備" if destination == "reserve" else "營地"], 2.2)
			update_bonds()
			play_sfx("ui_confirm")
			...
			show_message("%s調至主動欄，技能由完整冷卻開始。" % heroes[hid]["name"], 2.5)
			update_bonds()
			play_sfx("ui_confirm")
		else:
...
func draw_hud() -> void:
	...
	# Boss大招才使用血條下方的中央警示，與主動名將頭上喊招分流。
	...
	# 中下：主動名將採低高度技能列，保留戰場視野。
	draw_panel(HUD_HERO_RAIL_RECT, Color(0.018, 0.024, 0.025, 0.78), Color(0.40, 0.36, 0.26, 0.65), 1.0)
	var slot_count: int = max(1, active_limit())
	var card_gap: float = 7.0
	var usable_width: float = HUD_HERO_RAIL_RECT.size.x - 14.0
	var card_w: float = min(158.0, (usable_width - card_gap * float(slot_count - 1)) / float(slot_count))
	var card_h: float = 42.0
	var total: float = card_w * float(slot_count) + card_gap * float(slot_count - 1)
	var start_x: float = HUD_HERO_RAIL_RECT.position.x + (HUD_HERO_RAIL_RECT.size.x - total) * 0.5
	var card_y: float = HUD_HERO_RAIL_RECT.position.y + 7.0
	for i in range(slot_count):
		var card: Rect2 = Rect2(start_x + i * (card_w + card_gap), card_y, card_w, card_h)
		if i < active_heroes.size():
			var hero_id: String = str(active_heroes[i])
			draw_panel(card, Color(0.05, 0.055, 0.06, 0.94), heroes[hero_id]["color"], 1.5)
			var portrait_rect: Rect2 = Rect2(card.position + Vector2(4, 4), Vector2(32, 34))
			draw_texture_contain(portrait_tex[hero_id], portrait_rect)
			var text_x: float = card.position.x + 40.0
			var text_w: float = max(48.0, card.size.x - 50.0)
			draw_text("%d %s Lv.%d" % [i + 1, heroes[hero_id]["name"], int(hero_levels.get(hero_id, 1))], Vector2(text_x, card.position.y + 17), 12, Color8(239, 224, 187), true, HORIZONTAL_ALIGNMENT_LEFT, text_w)
			...
			var track: Rect2 = Rect2(text_x, card.position.y + 25, text_w - 4, 6)
			draw_rect(track, Color8(48, 52, 51), true)
			...
			draw_text("可施放" if cooldown <= 0.0 else "%.1fs" % cooldown, Vector2(text_x, card.position.y + 40), 9, Color8(150, 220, 160) if cooldown <= 0.0 else Color8(205, 181, 133), false, HORIZONTAL_ALIGNMENT_LEFT, text_w)
		else:
			draw_panel(card, Color(0.04, 0.045, 0.048, 0.82), Color8(75, 79, 75), 1.0)
			draw_centered_text("空位", card, 27.0, 12, Color8(116, 121, 116))
...
func draw_hero_config_screen() -> void:
	...
	draw_panel(
		Rect2(72, 50, 1136, 620), Color(0.03, 0.036, 0.038, 0.985), Color8(182, 151, 89), 2.0
	)
	draw_text("名將整備", Vector2(108, 98), 34, Color8(239, 214, 156), true)
	draw_text(
		"主戰 %d／%d｜後備 %d／%d｜營地 %d" % [active_heroes.size(), active_limit(), reserve_heroes.size(), reserve_limit(), camp_heroes.size()],
		Vector2(1165, 94), 17, Color8(193, 201, 193), true, HORIZONTAL_ALIGNMENT_RIGHT, 520
	)
	# 上方固定顯示目前主動陣容，避免換將時失去整體判讀。
	draw_text("目前主動陣容", Vector2(108, 132), 18, Color8(217, 197, 151), true)
	var slot_w: float = 174.0
	for slot_i in range(active_limit()):
		var slot_rect: Rect2 = Rect2(108 + slot_i * (slot_w + 10.0), 144, slot_w, 58)
		draw_panel(slot_rect, Color(0.043, 0.048, 0.048, 0.96), Color8(112, 103, 79), 1.0)
		if slot_i < active_heroes.size():
			var slot_id: String = str(active_heroes[slot_i])
			draw_texture_contain(portrait_tex[slot_id], Rect2(slot_rect.position + Vector2(5, 5), Vector2(45, 48)))
			draw_text(str(heroes[slot_id]["name"]), slot_rect.position + Vector2(57, 26), 16, heroes[slot_id]["color"], true, HORIZONTAL_ALIGNMENT_LEFT, 108)
			draw_text("Lv.%d" % int(hero_levels.get(slot_id, 1)), slot_rect.position + Vector2(57, 46), 12, Color8(177, 186, 177))
		else:
			draw_centered_text("空位", slot_rect, 35.0, 15, Color8(126, 132, 126))
	var order: Array[String] = known_hero_order()
	if order.is_empty():
		draw_text("尚未結識任何名將。", Vector2(108, 245), 22, Color8(196, 202, 195))
		return
	hero_config_index = clampi(hero_config_index, 0, order.size() - 1)
	var list_panel: Rect2 = Rect2(108, 220, 498, 370)
	draw_panel(list_panel, Color(0.038, 0.043, 0.044, 0.96), Color8(102, 96, 76), 1.2)
	draw_text("已結識名將", list_panel.position + Vector2(18, 30), 20, Color8(225, 205, 160), true)
	var visible_count: int = 6
	...
	for i in range(start, finish):
		...
		var row: int = i - start
		var rect: Rect2 = Rect2(list_panel.position.x + 12, list_panel.position.y + 46 + row * 50, list_panel.size.x - 24, 44)
	...
	draw_texture_contain(portrait_tex[selected_id], Rect2(detail_panel.position + Vector2(20, 24), Vector2(170, 215)))
	draw_text(str(heroes[selected_id]["name"]), detail_panel.position + Vector2(215, 55), 31, heroes[selected_id]["color"], true, HORIZONTAL_ALIGNMENT_LEFT, 280)
	draw_text(str(heroes[selected_id]["title"]), detail_panel.position + Vector2(215, 88), 18, Color8(223, 212, 179), true, HORIZONTAL_ALIGNMENT_LEFT, 280)
	draw_text("主動技能", detail_panel.position + Vector2(215, 126), 16, Color8(228, 204, 150), true)
	...
	var action: String = "移至後援" if active_heroes.has(selected_id) else ("調至主戰" if active_heroes.size() < active_limit() else "選擇替換主戰名將")
...
func draw_boss_loot_screen() -> void:
	...
	draw_text("敵將擊破", Vector2(640, 125), 43, Color8(244, 210, 132), true, HORIZONTAL_ALIGNMENT_CENTER, 700)
	draw_text(str(pending_boss_loot.get("boss_name", "敵將")), Vector2(640, 165), 26, Color8(222, 217, 199), true, HORIZONTAL_ALIGNMENT_CENTER, 700)
	...
	if relic_id != "" and relic_defs.has(relic_id):
		...
		draw_text("%s　Lv.%d" % [str(relic_defs[relic_id]["name"]), relic_level(relic_id) + 1 if has_relic(relic_id) else 1], left.position + Vector2(187, 205), 22, relic_rarity_color(str(relic_defs[relic_id].get("rarity","common"))), true, HORIZONTAL_ALIGNMENT_CENTER, 320)
		draw_wrapped(str(relic_defs[relic_id]["desc"]), Rect2(left.position + Vector2(28, 215), Vector2(320,45)), 13, Color8(193,200,192), 18.0, true)
	...
	if equipment_id != "" and equipment_defs.has(equipment_id):
		...
		draw_text("%s｜%s" % [equipment_slot_name(str(edef["slot"])), edef["name"]], right.position + Vector2(187, 205), 22, relic_rarity_color(str(edef.get("rarity","common"))), true, HORIZONTAL_ALIGNMENT_CENTER, 340)
		draw_wrapped(str(edef["desc"]), Rect2(right.position + Vector2(28, 215), Vector2(320,45)), 13, Color8(193,200,192), 18.0, true)
	else:
		draw_centered_text("本次未掉落裝備", right, 145.0, 18, Color8(146,153,146))
	draw_text("銅錢 +%d" % int(pending_boss_loot.get("coins",0)), Vector2(640, 515), 21, Color8(239,202,105), true, HORIZONTAL_ALIGNMENT_CENTER, 400)
