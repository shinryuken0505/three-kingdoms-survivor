...
| A17-ARCH-006 畫面與輸入路由 | 基礎完成／待本機驗證 | ScreenRouter、InputRouter 與生命週期、返回堆疊、按鍵正規化、Modal 輸入鎖已建立；尚未接管 `main.gd` | `1cd4f9b`、`68321ec`、`34c3c97` |
...
- 戰鬥規則：未變更。
- 玩家可見文字：未新增。
- `main.gd`：未接線，現有操作行為不應改變。
- Godot 本機 F5：待使用者驗證。
- 功能驗收：待後續逐畫面接入 Router 後進行。
